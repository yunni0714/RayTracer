...
describe('computeLaser — 세그먼트 hit 메타 (아트 클리핑용)', () => {
  it('표적 흡수 세그먼트는 hit.terminal=true', () => {
    const g = emptyGrid();
    g[2][0] = piece('ray', 90);        // dir 0
    g[2][2] = piece('target', 270);    // rel 90 충족면 — 흡수
    const r = computeLaser(g);
    const end = r.segments.find(s => s.x2 === 2 && s.y2 === 2)!;
    expect(end.hit).toEqual({ type: 'target', rotation: 270, terminal: true });
    expect(end.partial).toBe(false);
  });

  it('표면 차단(partial)은 hit 기물 정보를 갖고 terminal=false', () => {
    const g = emptyGrid();
    g[2][0] = piece('ray', 90);
    g[2][2] = piece('high_block');     // 완전 차단 (partial)
    const r = computeLaser(g);
    const end = r.segments.find(s => s.x2 === 2 && s.y2 === 2)!;
    expect(end.partial).toBe(true);
    expect(end.hit).toEqual({ type: 'high_block', rotation: 0, terminal: false });
  });

  it('반사(이어지는 빔 있음)는 terminal=false, 빈 셀은 hit 없음', () => {
    const g = emptyGrid();
    g[2][0] = piece('ray', 90);
    g[2][2] = piece('mirror');         // 반사 — 빔 계속
    const r = computeLaser(g);
    const bounce = r.segments.find(s => s.x2 === 2 && s.y2 === 2)!;
    expect(bounce.hit?.terminal).toBe(false);
    const pass = r.segments.find(s => s.x2 === 1 && s.y2 === 2)!;
    expect(pass.hit).toBeUndefined();
  });
});
